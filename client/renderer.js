  ...
  function attachReplay(options) {
    ...
    makeRenderer(options.canvas, options.assetBase, function (renderer) {
      ...
      if (options.playButton) {
        ...
        // The fixture re-attaches to the same transport bar, so any row a
        // previous attach inserted is removed first.
        var stale = options.playButton.parentNode.querySelector(".tspeed");
      ...
      // while the viewer is typing somewhere. The listener outlives its
      // attach (the fixture re-attaches per game and per width), so a stale
      // generation's listener does nothing.
