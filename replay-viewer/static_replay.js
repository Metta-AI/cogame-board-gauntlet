// Board Gauntlet static replay shell: fetches the replay named by ?replay=<url>,
...
(function () {
  ...
  function start(module, bytes) {
    ...
    var ok = module._bg_load_replay(ptr, bytes.length);
    module._free(ptr);
    if (!ok) {
      fail(readString(module, module._bg_error_ptr(),
        module._bg_error_len()) || "wasm rejected the replay");
      return;
    }
    var payload = JSON.parse(
      readString(module, module._bg_payload_ptr(),
        module._bg_payload_len())
    );
    var loading = document.getElementById("loading");
    ...
    GauntletRenderer.attachReplay({
      ...
      assetBase: "./assets",
      payload: payload,
      // BOARD-GAUNTLET: the renderer sets data-replay-loaded="true" inside
      // this callback, on its FIRST DRAWN FRAME, and only then do we post
      // `ready`. The starter posted `ready` from a double
      // requestAnimationFrame at this call site, which can fire before the
      // first painted frame and let an embedding page sample an unpainted
  ...
  function load() {
    ...
    caption(attempt > 1 ? "RETRYING REPLAY… (attempt " + attempt + ")" :
      "LOADING REPLAY…");
    if (!modulePromise) {
      modulePromise = GauntletReplayModule().catch(function (error) {
